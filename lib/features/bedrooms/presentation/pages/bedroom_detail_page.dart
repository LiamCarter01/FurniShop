...
class _BedroomDetailPageState extends ConsumerState<BedroomDetailPage> {
  ...
  Widget _buildDetailContent({
    ...
    return Column(
      children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: AppText(
                          text: _formatText(item.category),
                      ...
                      Row(
                        ...
                        children: [
                          AppText.price(amount: item.price),
                          if (item.hasDiscount) ...[
                            const SizedBox(width: 8),
                            AppText.originalPrice(amount: item.originalPrice!),
                            const SizedBox(width: 8),
                            Container(
                              padding: const EdgeInsets.symmetric(
                                horizontal: 8,
                      ...
                      const SizedBox(height: 12),
                      _buildSpecRow('Material', _formatText(item.material)),
                      _buildSpecRow('Color', _formatText(item.color)),
                      _buildSpecRow('Dimensions', item.dimensions.formatted),
                      const SizedBox(height: 24),
        ...
        _buildBottomBar(item, quantity, totalPrice),
      ],
    );
  }

  Widget _buildImageSection(Bedroom item) {
    return Stack(
      children: [
        AspectRatio(
          aspectRatio: 1,
  ...
  Widget _buildBottomBar(Bedroom item, int quantity, double totalPrice) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
            ...
            Expanded(
              child: AppButton(
                ...
                onPressed: item.inStock
                    ? () {
                        _showAddToCartSnackbar(item.name, quantity);
                      }
                    : null,
